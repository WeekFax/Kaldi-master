...
class KaldiXVectorExtraction(object):
    ...
    def __init__(self, nnet, binary_path=nnet3bin_path, use_gpu=False,
        ...
        Args:
            ...
            cache_capacity (int):
        """
        self.nnet3_compute=os.path.join(binary_path, 'nnet3-compute')
        if not os.path.exists(self.nnet3_compute):
            raise ValueError(
                'Path to nnet3-compute - `{}` does not exists.'.format(self.nnet3_compute))
    ...
    def features2embeddings(self, data_dict):
        ...
        with tempfile.NamedTemporaryFile(delete=False) as xvec_ark, tempfile.NamedTemporaryFile(delete=False) as mfcc_ark:
            ...
            '''
            with open(mfcc_ark.name, 'w') as f:
                f.write('[')
                for row_idx in range(data_dict.shape[0]):
                    f.write('{}  '.format(os.linesep))
                    data_dict[row_idx].tofile(f, sep=' ', format='%.6f')
                f.write(' ]{}'.format(os.linesep))
            '''
            ...
            #args=[self.nnet3_compute, self.nnet, 'ark,t:{}'.format(mfcc_ark.name), 'ark,t:{}'.format(xvec_ark.name)]
    ...
    def audio2features(self, input_path):
        ...
        with tempfile.NamedTemporaryFile() as wav_scp, tempfile.NamedTemporaryFile(delete=False) as mfcc_ark:
            ...
            args = [self.compute_mfcc_feats_bin,
                    ...
                    'ark{}'.format(',t:{}'.format(mfcc_ark.name) if not self.apply_cmvn_sliding else 'p:-')]
...
class VoiceActivityDetector:
    def __init__(self, vad=webrtcvad.Vad(2), filter_order=1, filter_frequency=0.0002, threshold=0.2):
        """
    ...
    def get_frames(self, s: np.ndarray, frame_length):
        n_frames = s.shape[0] // frame_length
        remainder = s.shape[0] % frame_length
        if remainder > 0:
            s = np.int16(s[0:-remainder] * 2 ** 15)
        else:
            s = np.int16(s * 2 ** 15)
        return np.reshape(s, (n_frames, frame_length))

    def get_mask(self, s: np.ndarray):
        frame_length = self.sample_rate // 1000 * self.frame_length_ms
        frames = self.get_frames(s, frame_length=frame_length)

        speech = [self.vad.is_speech(sample.tobytes(), sample_rate=self.sample_rate)
        for sample in frames for _ in range(frame_length)]
        filtered_speech = filtfilt(self.filter_b, self.filter_a, speech)

        return np.where(filtered_speech > self.threshold)

    def get_speech(self, s: np.ndarray):
